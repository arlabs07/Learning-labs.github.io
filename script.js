...
let focusedElementBeforeModal = null; // For accessibility: store element that triggered modal
...
const renderPageList = () => {
    ...
    pages.forEach(page => {
        ...
        li.className = 'mb-2';
        li.innerHTML = `
            <div class="flex items-center justify-between bg-gray-700 hover:bg-gray-600 rounded-md p-2 cursor-pointer page-item transition-all duration-200 ease-in-out hover:scale-[1.02]" data-page-id="${page.id}" role="button" tabindex="0" aria-label="Go to ${page.name} page">
                <span class="flex-grow text-lg font-medium page-name-display truncate">${page.name}</span>
                <button class="delete-page-button text-gray-400 hover:text-red-400 ml-2 p-1 rounded-full hover:bg-gray-500 transition-all duration-200 ease-in-out transform hover:scale-110" aria-label="Delete ${page.name} page">
...
const renderMathFormulas = () => {
    ...
    document.querySelectorAll('.formula-block .katex-display, .formula-block .katex').forEach(el => el.remove());
    // Re-render all math elements within formula-block
    document.querySelectorAll('.formula-block').forEach(block => {
        const originalText = block.textContent; // Get the raw LaTeX text
        // Clear the block and re-render
        block.innerHTML = originalText; // Put back the original text for re-rendering
...
const openInputModal = (type) => {
    focusedElementBeforeModal = document.activeElement; // Save focused element
    ...
    switch (type) {
        case 'header':
            fieldsHtml = `
                ...
                <input type="text" id="headerText" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="My Section Title" value="New Header" aria-label="Header Text">
                <label class="block text-gray-300 text-sm font-bold mb-2 mt-4" for="headerLevel">Header Level (1-6):</label>
                <input type="number" id="headerLevel" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" min="1" max="6" value="2" aria-label="Header Level">
            `;
            break;
        case 'paragraph':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="paragraphText">Paragraph Text:</label>
                <textarea id="paragraphText" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline h-32" placeholder="This is a new paragraph." aria-label="Paragraph Text"></textarea>
            `;
            break;
        case 'list':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="listItems">List Items (one per line):</label>
                <textarea id="listItems" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline h-32" placeholder="Item 1\nItem 2\nItem 3" aria-label="List Items"></textarea>
                <div class="flex items-center mt-4">
                    <input type="checkbox" id="orderedList" class="mr-2 leading-tight" aria-label="Ordered List">
        ...
        case 'italic':
            fieldsHtml = `
                ...
                <input type="text" id="textInput" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="Enter text here" value="${type.charAt(0).toUpperCase() + type.slice(1)} Text" aria-label="Text to format">
            `;
            break;
        case 'image':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="imageUrl">Image URL:</label>
                <input type="url" id="imageUrl" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="https://example.com/image.jpg" value="https://placehold.co/400x200/cccccc/000000?text=Image" aria-label="Image URL">
                <label class="block text-gray-300 text-sm font-bold mb-2 mt-4" for="imageAlt">Alt Text (for accessibility):</label>
                <input type="text" id="imageAlt" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="Description of image" aria-label="Image Alt Text">
            `;
            break;
        case 'video':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="videoUrl">Video URL (MP4 or YouTube embed link):</label>
                <input type="url" id="videoUrl" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="https://example.com/video.mp4 or YouTube URL" value="https://www.w3schools.com/html/mov_bbb.mp4" aria-label="Video URL">
            `;
            break;
        case 'pdf':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="pdfUrl">PDF URL:</label>
                <input type="url" id="pdfUrl" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="https://example.com/document.pdf" value="https://www.africau.edu/images/default/sample.pdf" aria-label="PDF URL">
            `;
            break;
        case 'iframe':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="iframeUrl">Website URL for IFrame:</label>
                <input type="url" id="iframeUrl" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="https://example.com" value="https://www.google.com/webhp?igu=1" aria-label="Iframe URL">
            `;
            break;
        case 'table':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="tableRows">Number of Rows:</label>
                <input type="number" id="tableRows" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" min="1" value="3" aria-label="Number of Table Rows">
                <label class="block text-gray-300 text-sm font-bold mb-2 mt-4" for="tableCols">Number of Columns:</label>
                <input type="number" id="tableCols" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" min="1" value="2" aria-label="Number of Table Columns">
            `;
            break;
        case 'code':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="codeLanguage">Code Language (e.g., javascript, python, html):</label>
                <input type="text" id="codeLanguage" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="javascript" value="javascript" aria-label="Code Language">
                ...
                <input type="text" id="codeFileName" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" placeholder="example.js" value="example.js" aria-label="Code File Name">
                <label class="block text-gray-300 text-sm font-bold mb-2 mt-4" for="codeContent">Code Content:</label>
                <textarea id="codeContent" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline h-48" placeholder="// Your code goes here" aria-label="Code Content"></textarea>
            `;
            break;
        case 'formula':
            fieldsHtml = `
                <label class="block text-gray-300 text-sm font-bold mb-2" for="formulaText">LaTeX Formula (use $$ for display, $ for inline):</label>
                <textarea id="formulaText" class="shadow appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline h-32" placeholder="$$E=mc^2$$" aria-label="LaTeX Formula"></textarea>
            `;
            break;
    }
    inputModalFields.innerHTML = fieldsHtml;
    inputModal.style.display = 'flex'; // Show the input modal
    inputModal.focus(); // Focus the modal for accessibility
};
...
const handleInputModalSubmit = () => {
                ...
                contentHtml = `
                    <div class="code-block">
                        <div class="code-header">
                            ...
                            <button class="copy-button" onclick="copyCode(this)" aria-label="Copy code to clipboard"><i class="fas fa-copy"></i></button>
                        </div>
    ...
    if (focusedElementBeforeModal) {
        focusedElementBeforeModal.focus(); // Restore focus
    }
};
...
const showConfirmationModal = (message, title = 'Confirm Action', callback) => {
...
pagesContainer.addEventListener('click', (e) => {
    ...
    if (pageItem) {
        ...
        if ((isGeneratorMode || isEditModeActive) && e.target.closest('.delete-page-button')) {
            ...
            showConfirmationModal('Are you sure you want to delete this page?', 'Delete Page', (confirmed) => {
                if (confirmed) {
            ...
            if (window.innerWidth < 1024) { // Close sidebar on mobile after page selection
...
const generateAndDownloadCurrentHTML = () => {
    ...
    // Remove the script tag linking to script.js from the downloaded HTML
    htmlContent = htmlContent.replace(/<script src="script\.js"><\/script>/, '');

    // Inject current state into the downloaded HTML within a new script block
    const injectedDataScript = `
        <script>
            ...
            // Re-run initialization for the hosted site
            document.addEventListener('DOMContentLoaded', () => {
                // Re-initialize all necessary elements and state for the hosted site
                // This part mimics the initial setup but uses the injected 'initial' data
                const websiteNameInput = document.getElementById('websiteNameInput');
                const headerLogo = document.getElementById('headerLogo');
                const sidebarLogo = document.getElementById('sidebarLogo');
                const pagesContainer = document.getElementById('pagesContainer');
                ...
                let pages = initialPagesData;
                let currentPageId = 'home'; // Default to 'home' page
                let isGeneratorMode = initialIsGeneratorMode;
                let isEditModeActive = initialIsEditModeActive;
                let usedUrls = initialUsedUrlsData;

                // Helper to escape HTML for code blocks (needed in hosted site too)
                const escapeHtml = (unsafe) => {
                    return unsafe
                        .replace(/&/g, "&amp;")
                        .replace(/</g, "&lt;")
                ...
                // Functions needed for hosted site (subset of original script.js)
                const renderPageList = () => {
                ...
                const loadPageContent = (pageId) => {
                    ...
                    if (page) {
                        ...
                        console.error('Page not found:', pageId);
                        // Fallback in case no pages are defined in initialPagesData
                        if (pages.length === 0) {
                            pages.push({ id: 'home', name: 'Home', content: '<p>Welcome to your new page!</p>' });
                        }
                        loadPageContent('home');
                    }
                    applyModeAndEditState(); // Apply state after content load
                };
                ...
                const applyModeAndEditState = () => {
                    ...
                    if (window.innerWidth < 1024) {
                        // Mobile: Sidebar toggleable via menu button
                    } else {
                        sidebar.classList.add('open'); // Desktop: Sidebar always open
                    }
                };

                // Initialize the hosted site
                websiteNameInput.value = initialWebsiteName;
                headerLogo.src = initialWebsiteLogoUrl;
                sidebarLogo.src = initialWebsiteLogoUrl;

                // Event Listeners for Hosted Site (only necessary ones)
                pagesContainer.addEventListener('click', (e) => {
                    const pageItem = e.target.closest('.page-item');
                    if (pageItem && !e.target.closest('.delete-page-button')) {
                        loadPageContent(pageItem.dataset.pageId);
                        if (window.innerWidth < 1024) { // Close sidebar on mobile after page selection
                            sidebar.classList.remove('open');
                        }
                    }
                ...
                toggleEditModeButton.addEventListener('click', () => {
                    ...
                    // In hosted mode, save changes to localStorage
                    if (isEditModeActive) {
                        localStorage.setItem('websitePages', JSON.stringify(pages));
                        localStorage.setItem('websiteName', websiteNameInput.value);
                        localStorage.setItem('websiteLogoUrl', headerLogo.src);
                        localStorage.setItem('usedUrls', JSON.stringify(usedUrls));
                    }
                    isEditModeActive = !isEditModeActive;
                    localStorage.setItem('isEditModeActive', isEditModeActive); // Persist edit state
                    applyModeAndEditState();
                });

                // For hosted site, if it's in edit mode, allow saving changes
                if (isEditModeActive) {
                    websiteNameInput.addEventListener('input', () => {
                        // No direct save here, handled by toggleEditModeButton click
                    });
                    ...
                    currentPageNameInput.addEventListener('input', () => {
                        const pageIndex = pages.findIndex(p => p.id === currentPageId);
                        if (pageIndex !== -1) {
                            pages[pageIndex].name = currentPageNameInput.value;
                            renderPageList(); // Update sidebar page name instantly
                        }
                    });
                }

                // Initial load for the hosted site
    ...
    // Find the closing </body> tag and insert the new script block before it
    htmlContent = htmlContent.replace('</body>', `${injectedDataScript}\n</body>`);
...
window.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') {
        if (contentTypeModal.style.display === 'flex') {
            closeContentTypeModalButton.click();
        } else if (inputModal.style.display === 'flex') {
            closeInputModalButton.click();
        } else if (logoModal.style.display === 'flex') {
            closeLogoModalButton.click();
